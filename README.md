...
**For each REMOTE:**
3. On the REMOTE: install the driver — `install.ps1` (Windows) or `install-mac.sh` (macOS). **No arguments needed.** Both generate an SSH key if one isn't there (`%USERPROFILE%\.ssh\id_clipsync` on Windows), ask for LOCAL's hostname/IP, and write the `clipsync-local` alias. The SSH test at the end **fails on a first run** — the key isn't authorized on LOCAL yet — and `install.ps1` prints the exact command for step 4.
4. Back on LOCAL **(elevated)**: authorize that key — `setup-ssh-account.ps1 -AddKeyOnly <pubkey>`.
5. Verify from the REMOTE: `ssh clipsync-local hostname` (no password prompt), then a real hotkey pull **and** push (files, not just text).

**Finally — once every REMOTE works as `clipsync`:**
6. On LOCAL, remove `admin`'s key from `C:\ProgramData\ssh\administrators_authorized_keys` so `admin` is no longer SSH-reachable.
...
| `%USERPROFILE%\.ssh\id_clipsync` | clipsync-only SSH key, generated by `install.ps1` on first run |
